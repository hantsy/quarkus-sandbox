...
Lombok is used to erase the setters and getters for your POJO classes. I have used frequently in my project.

## Declaring  GraphQL Client API

Similar to the MicroProfile RestClient API, SmallRye GraphQL provides the same approach to declare a GraphQL Client from an interface.
...
The  POJO classes, such as `Post`,  `Coment` and  `CreatePost` are copied from [the project](https://github.com/hantsy/quarkus-sandbox/blob/master/graphql) we created in the last post.

Similar to the MP RestClient API,  configure the base URI of the remote GraphQL API.

```properties
com.example.demo.PostGraphQLClient/mp-graphql/url=http://localhost:8080/graphql
```

Now let's try to call the `getAllPosts` to print all posts.  Create a bean implements `QuarkusApplication` and annotated it with `@QuarkusMain`, it will work like the main class in the general Java application.
...
You will see the following info in the application log.
...
Comparing to REST APIs, the most attractive feature of GraphQL is it only returns the required fields that are declared by client.  In the above the codes it returns all fields. 
...
Currently when calling the Client APIs, Quarkus does not provide an registry for handling exceptions like MP RestClient API, but there are some possible means to archive the purpose.
...
Besides this, wrapping your response with `ErrorOr` class.
...
`ErrorOr` is very similar to the Java `Optional`,  you can navigate one of two parts, **data** and **errors**. 
...
For example,  perform the query in the [GraphQL UI](http://localhost:8080/q/graphql-ui/) to retrieve all posts.
...
Through the `document`, `operation` and `field`  ,  we build the same query structure in Java *dialect*. 

The value of `NamedClient` to identify different clients,  which is used as the configuration key in the *application.properties*.

```properties
post-dynamic-client/mp-graphql/url=http://localhost:8080/graphql
```

Now try to call the  `getAllPosts` of this dynamic client and print out all posts.
...
The *text block* is great to compose a multiline string, but there is [an issue](https://github.com/quarkusio/quarkus/issues/17667) caused the schema parsing failed. I added `replaceAll` to erase the newline breaks.

Let's try to call the `getAllPosts` in this client.
...
Run the application you will see the following application log.
